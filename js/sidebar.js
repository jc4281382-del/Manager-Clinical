...
function openSidebar() {
  ...
  if (prof) {
    document.getElementById('sidebarName').innerText = prof.preferred_name || prof.full_name;
    document.getElementById('sidebarEmail').innerText = prof.email || '';
    if (prof.photo_url) document.getElementById('sidebarPhoto').src = prof.photo_url;
  }
}
...
window.openProfile = () => {
  ...
  const prof = window.currentProfessional;
  if (prof) {
    document.getElementById('prefName').value = prof.preferred_name || '';
    document.getElementById('fullName').value = prof.full_name || '';
    document.getElementById('workStart').value = prof.work_start || '08:00';
    document.getElementById('workEnd').value = prof.work_end || '18:00';
  }
...
window.uploadPhoto = async (input) => {
  ...
  document.getElementById('sidebarPhoto').src = publicUrl + '?t=' + Date.now();
  const pi = document.getElementById('profilePhoto');
  if (pi) pi.src = publicUrl + '?t=' + Date.now();
  Swal.fire({icon:'success',title:'Foto atualizada!',timer:2000,showConfirmButton:false});
};

window.openRelatorio = () => {
  closeSidebar();
  const m = document.getElementById('relatorioModal');
  m.classList.remove('hidden'); m.classList.add('flex');
};
...
window.downloadRelatorio = async (period) => {
  ...
  if (period === 'hoje') {
    ...
    from = document.getElementById('relatorioDateFrom').value;
    to = document.getElementById('relatorioDateTo').value;
    if (!from || !to) { Swal.fire({icon:'warning',title:'Atenção',text:'Selecione as datas.'}); return; }
  }
  // Offset de fuso para filtros corretos
  ...
  const { data } = await window.supabase.from('appointments')
    .select('scheduled_at, appointment_type, value, status, patients(full_name, phone)')
    .eq('professional_id', pid)
    .gte('scheduled_at', `${from}T00:00:00${tzStr}`)
    .lte('scheduled_at', `${to}T23:59:59${tzStr}`)
    .order('scheduled_at');
  ...
  data.forEach(a => {
    ...
    rows.push([date, time, a.patients?.full_name||'—', a.patients?.phone||'—', a.appointment_type, `R$ ${(a.value||0).toFixed(2)}`, a.status]);
    if (a.status === 'Realizado') total += (a.value || 0);
  });
  ...
  a.download = `relatorio-${from}-${to}.csv`; a.click();
