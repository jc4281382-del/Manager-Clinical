...
async function loadPacientes() {
  ...
  const { data, error } = await window.supabase
    .from('appointments')
    .select('id, scheduled_at, status, value, patients(id, full_name, phone, email)')
  ...
  STATUS_COLS.forEach(status => {
    ...
    col.innerHTML = items.map(a => {
      ...
      return `
      <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-3 space-y-2 hover:shadow-md transition-shadow cursor-default">
        <div class="flex items-center gap-2">
          ...
          <div class="min-w-0">
            ...
            <p class="text-xs text-on-surface-variant">${a.patients?.phone || ''}</p>
          </div>
        </div>
        ...
        <div class="flex gap-1 flex-wrap">
          ${STATUS_COLS.filter(s=>s!==status).map(s=>
            `<button onclick="moverPaciente('${a.id}','${s}')" class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 hover:bg-primary hover:text-white transition-colors">${s}</button>`
          ).join('')}
        </div>
      </div>`;
